...
pub fn NavBar() -> Html {
    ...
    html! {
        <nav class="flex justify-between items-center relative p-2 bg-inherit">
            <div class="flex items-center gap-4">
                ...
                <OverLadNavLink<Route> to={Route::Images} classes="max-sm:hidden">
                    <h2>{ "Images" }</h2>
            ...
            <div class={classes!("absolute", "top-full", "left-0", "right-0", "bg-inherit", "sm:hidden", "overflow-y-hidden", "duration-400", if nav_menu_state_reducer.shown { "h-64 border-y" } else { "h-0" })}>
